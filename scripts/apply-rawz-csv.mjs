/**
 * rawz-scraped.json → prisma/cat_food.csv 로우즈 보강
 * Usage: node scripts/apply-rawz-csv.mjs
 */
...
const scraped = JSON.parse(
  readFileSync(join(process.cwd(), "scripts", "rawz-scraped.json"), "utf8"),
);
...
console.log(`Updated ${updated} RAWZ rows in ${CSV_PATH}`);
