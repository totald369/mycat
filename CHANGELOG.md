...
### 사료 상세 — 잘못된 slug 301 리다이렉트

- `/foods/babiboyak-adult-tuna` → 더리얼 캣 그레인프리 크런치 참치 어덜트 1kg. 밥이보약에는 참치 어덜트 제품이 없고, 같은 하림 라인의 해당 SKU로 연결.

## 2026-08-13

### 사료 상세 SEO 부스트 — 프로뉴트리션 12종

- 프레스티지·퓨어라이프 12종 OpenAI SEO 부스트 생성(추천·급여 참고·비교 포인트) → `prisma/feedSeoBoost.json` 반영.
...
- [네이버 브랜드스토어](https://brand.naver.com/britkorea/products/10254007071) **브릿 케어 캣 그레인프리 키튼 성장 2kg** 신규 등록.
- Brit 공식 원재료·등록성분·ME(4000 kcal/kg) 반영 (`NV-10254007071`).
...
### 사료 상세페이지 Figma 디자인 반영

- [Figma 상세 화면](https://www.figma.com/design/xYm8i5QKVjyWWWlA6nLfIW/) 기준 UI: 4칸 칼로리·급여 카드, 추천 대상→성분→안내(아이콘) 순서, 관련 사료 5섹션, CTA·타이포·색상(`#f9f9f9` 카드) 정렬.

### 사료 상세페이지 중복 정보 정리

- SEO부스트 있을 때 「추천 대상」 칩 숨김(FAQ·JSON-LD는 유지).
- 통합 「관련 사료」 섹션 제거, Figma 5섹션(브랜드·목적·칼로리·연령)만 유지.
...
- `buildFeedDetailFaqs`: FAQ 3~4개로 축소, 사료 특성(실내·키튼·시니어·소화·비뇨·헤어볼·체중·습식·영양) 우선 선택.
- 키튼 연령·체중관리 등 무관한 템플릿 질문 제거, 긴 습식명 축약, 의료 주장(치료·예방·완치) 회피.
- slug·canonical·metadata·JSON-LD `FAQPage` 구조는 유지.

### 사료 상세 관련 사료 통합

- 4중 관련 사료 섹션을 「관련 사료」 1개로 통합(최대 8건, 중복 제거, `같은 브랜드` 등 라벨 표시).
